...
static void HandleVideo( demux_t *p_demux, const uint8_t *p_buffer )
{
    demux_sys_t *p_sys = p_demux->p_sys;
...
static void HandleAudio( demux_t *p_demux, const uint8_t *p_buffer )
{
    demux_sys_t *p_sys = p_demux->p_sys;
    int i;
...
static int InitCapture( demux_t *p_demux )
{
    ...
    if ( p_sys->i_max_channel != -1 )
    {
        ...
        for ( unsigned i = 0; i < p_sys->i_abuffers; i++ )
        {
    ...
    for ( unsigned i = 0; i < p_sys->i_vbuffers; i++ )
    {
...
static int Capture( demux_t *p_demux )
{
    ...
    if ( pfd[0].revents & POLLIN )
    {
        ...
        HandleVideo( p_demux, p_sys->pp_vbuffers[p_sys->i_current_vbuffer] );
    ...
    if ( p_sys->i_max_channel != -1 && pfd[1].revents & POLLIN )
    {
        ...
        HandleAudio( p_demux, p_sys->pp_abuffers[p_sys->i_current_abuffer] );
