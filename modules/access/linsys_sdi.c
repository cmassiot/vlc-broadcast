...
static void NewFrame( demux_t *p_demux )
{
    demux_sys_t *p_sys = p_demux->p_sys;
...
static void StartDecode( demux_t *p_demux )
{
    demux_sys_t *p_sys = p_demux->p_sys;
    es_format_t fmt;
    char *psz_parser;
...
static void DecodeTelx( demux_t *p_demux )
{
    demux_sys_t *p_sys = p_demux->p_sys;
...
static void InitAudio( demux_t *p_demux, sdi_audio_t *p_audio )
{
    demux_sys_t *p_sys = p_demux->p_sys;
    es_format_t fmt;
...
static void DecodeAudio( demux_t *p_demux, sdi_audio_t *p_audio )
{
    demux_sys_t *p_sys = p_demux->p_sys;
    block_t *p_block;
    int16_t *p_output;
...
static void DecodeFrame( demux_t *p_demux )
{
    demux_sys_t *p_sys = p_demux->p_sys;
    int i;
    ...
    for ( i = 0; i < MAX_AUDIOS; i++ )
    {
        if ( p_sys->p_audios[i].i_group && p_sys->p_audios[i].p_es != NULL )
            DecodeAudio( p_demux, &p_sys->p_audios[i] );
    }
                               ...
                               const uint8_t *p_end )
{
    ...
    if ( p_end - *pp_parser < i_total_size )
    {
